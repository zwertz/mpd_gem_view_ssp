...
// plot pedestal
void pedestal_compare_no6chan(int run1,int run2,const char *cable_length1,const char *cable_length2)
{
  ...
  const char* path = InPath.c_str();
    ...
    TH1F *h_res1 = new TH1F("h_res1","RMS of All Channels;ADC RMS;",167,0,50);
 ...
 fstream g(mypath, fstream::in);
    ...
    TH1F *h_res2 = new TH1F("h_res2","RMS of All Channels;ADC RMS;",167,0,50);
    ...
    // save histos
    TFile *froot1 = new TFile(Form(outDir + "pedestal_no6chan_%i.root",run1), "recreate");
    for(auto &i: res1)
        i->Write();
...
// save histos
    TFile *froot2 = new TFile(Form(outDir + "pedestal_no6chan_%i.root",run2), "recreate");
    for(auto &j: res2)
        j->Write();
    ...
    TFile *f_pedestal1 = new TFile(Form(outDir + "pedestal_no6chan_%i.root",run1));
    TFile *f_pedestal2 = new TFile(Form(outDir + "pedestal_no6chan_%i.root",run2));
    ...
    TString output = Form(outDir + "pedestal_no6chan_%i_%i.pdf",run1,run2);
