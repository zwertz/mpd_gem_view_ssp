...
// plot pedestal
void pedestal_compare_with6chan(int run1,int run2, const char *cable_length1,const char *cable_length2)
{
  ...
  const char* path = InPath.c_str();
    ...
    TH1F *h_res1 = new TH1F("h_res1","RMS of All Channels;ADC RMS;",233,0,70);
 ...
 fstream g(mypath, fstream::in);
    ...
    TH1F *h_res2 = new TH1F("h_res2","RMS of All Channels;ADC RMS;",233,0,70);
    ...
    // save histos
    TFile *froot1 = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run1), "recreate");
    for(auto &i: res1)
        i->Write();
...
// save histos
    TFile *froot2 = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run2), "recreate");
    for(auto &j: res2)
        j->Write();
    ...
    rms_legend->AddEntry(h_res1,cable_length1,"l");
    rms_legend->AddEntry(h_res2,cable_length2,"l");
    rms_legend->Draw();
    ...
    off_legend->AddEntry(h_off1,cable_length1,"l");
    off_legend->AddEntry(h_off2,cable_length2,"l");
    off_legend->Draw();
    ...
    TFile *f_pedestal1 = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run1));
    TFile *f_pedestal2 = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run2));
	...
	if(temp_MPD != current_MPD1){
	  ...
	  h1->Draw("same");
          ...
          //Give histogram a legend
           ...
           legend_rms->AddEntry(h1,cable_length1,"l");
           legend_rms->AddEntry(h2,cable_length2,"l");
           legend_rms->Draw();
	...
	if(title.find("offset") != string::npos){
	  ...
	  h2->Draw("sames");
          ...
          auto legend_off = new TLegend(0.3,0.80,0.7,0.9);
           ...
           legend_off->AddEntry(h1,cable_length1,"l");
           legend_off->AddEntry(h2,cable_length2,"l");
           legend_off->Draw();
    ...
    TString output = Form(outDir + "pedestal_with6chan_%i_%i.pdf",run1,run2);
