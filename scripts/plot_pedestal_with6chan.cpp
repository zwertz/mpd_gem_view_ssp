...
// plot pedestal
void plot_pedestal_with6chan(int run,const char *cable_length)
{
  ...
  const char* path = InPath.c_str();
    ...
    TH1F *h_res = new TH1F("h_res","RMS of All Channels;ADC RMS;",233,0,70);
    ...
    TFile *froot = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run), "recreate");
    for(auto &i: res)
        i->Write();
    h_res->Write();
    h_off->Write(); 
   ...
   froot->Close();	
    ...
    TFile *f_pedestal = new TFile(Form(outDir + "pedestal_with6chan_%i.root",run));
    ...
    TString output = Form(outDir + "pedestal_with6chan_%i.pdf",run);
